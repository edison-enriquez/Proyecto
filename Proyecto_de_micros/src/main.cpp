...
void setup() {
  ...
  if (!display.begin(SSD1306_SWITCHCAPVCC, 0x3C)) {
    ...
    while (true);  // Faltaba punto y coma
  }
...
void loop() {
  ...
  // Mapear valores de joystick a ángulos de servos
  valX = map(xValue, 0, 4095, 0, 180);
  valY = map(yValue, 0, 4095, 0, 180);

  // Mover servos según el joystick
  servoX.write(valX);
  servoY.write(valY);

  // Servo Z (presionar el botón para "dibujar")
  if (swValue == LOW) {
    servoZ.write(45);  // Baja el marcador
    movimiento = "Dibujando!";
  } else {
    servoZ.write(90);  // Sube el marcador
  }
